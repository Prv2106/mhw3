...
function petAdoptionRequest(event){
    ...
    let petfinder_request = petfinder_api_endpoint_organization + "?query=" + encoded_text + '&limit=' + num_results;
    
    fetch(petfinder_request,
            {
                headers:
                {
...
// API senza api_key
const num_zoo_animals = 7;
const zoo_api_endpoint = 'https://zoo-animal-api.herokuapp.com';
...
function zooRequest(){
    const zoo_request = zoo_api_endpoint + '/animals/rand/' + num_zoo_animals;
    fetch(zoo_request).then(onResponse).then(onZooJson);
}
